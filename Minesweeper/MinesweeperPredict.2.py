...
def hit_cell(action):
    ...
    if i > 8 or j > 8:
        ...
        return
    mouse.position = (15 + 16 * i + 8, 105 + 16 * j + 8)
    time.sleep(.05)
...
while True:
    ...
    while not done:
        ...
        ########################################################################################################
        # Enter learning here
        action_qs = model.predict(np.array(state).reshape(-1, *state.shape))[0]
        # print(action_qs)
        valid_act = False
        i, j = 0, 0
        action = -1
        while not valid_act:
            ...
            # print(action)
            (j, i) = divmod(action, 9)
            if cell_recognition(scs, i, j) == 0:
                valid_act = True
                break
            action_qs[action] = float("-inf")
        # print("would choose: ", action)
        # valid_act = False
        # while not valid_act:
...
print("100 games -> won: ", games_won, " | maximum score: ", max_reward, " | avg score: ", sum_reward/100, ".")
